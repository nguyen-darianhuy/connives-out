...
namespace VRInteraction
{
    public class VRInteractor : MonoBehaviour
    {
        ...
        public virtual VRInput vrInput
        {
            get
            {
                ...
                if (_vrInput == null)
                    Debug
                        .LogError("VRInteractor needs an input component",
                        gameObject);
                return _vrInput;
            }
        }

        public virtual VRInteractableItem hoverItem
        {
            get
            {
                return _hoverItem;
        ...
        public virtual VRInteractableItem heldItem
        {
            get
            {
                return _heldItem;
        ...
        public virtual Transform GetVRRigRoot
        {
            get
            {
#if Int_SteamVR
                if (vrInput.isSteamVR())
                {
                    if (_vrRigRoot == null)
                    {
                        SteamVR_PlayArea playArea =
                            GetComponentInParent<SteamVR_PlayArea>();
                        if (playArea != null)
                ...
                if (!vrInput.isSteamVR())
                {
                    if (_vrRigRoot == null)
                    {
                        ...
                        if (ovrAvatar != null)
                        {
                            if (ovrAvatar.transform.parent != null)
                                _vrRigRoot = ovrAvatar.transform.parent;
                            else
                                _vrRigRoot = ovrAvatar.transform;
                        }
                        else
                            _vrRigRoot = transform.root;
                    }
        ...
        public virtual Vector3 Velocity
        {
            get
            {
                ...
                if (vrInput.isSteamVR())
                {
                    ...
                    var device =
                        SteamVR_Controller
                            .Input((int) vrInput.controller.controllerIndex);
                ...
                if (!vrInput.isSteamVR())
                {
                    return GetVRRigRoot.rotation *
                    OVRInput.GetLocalControllerVelocity(vrInput.controllerHand);
                }
#endif
            ...
            get
            {
                ...
                if (!vrInput.isSteamVR())
                {
                    Quaternion deltaRotation =
                        (_currentRotation * Quaternion.Inverse(_lastRotation));
                    return new Vector3(Mathf
                            .DeltaAngle(0, deltaRotation.eulerAngles.x),
        ...
        public virtual void TriggerHapticPulse(int frames)
        {
        ...
        public virtual Transform getControllerAnchor
        {
            get
            {
                if (controllerAnchor == null) controllerAnchor = transform;
                return controllerAnchor;
            }
        }

        public virtual Transform getControllerAnchorOffset
        {
            get
            {
                if (controllerAnchorOffset == null)
                {
                    GameObject controllerAnchorObject =
                        new GameObject(VRInteractor.anchorOffsetName);
                    controllerAnchorOffset = controllerAnchorObject.transform;
                    controllerAnchorOffset.parent = getControllerAnchor;
                    if (vrInput.isSteamVR())
                    {
                        controllerAnchorOffset.localPosition =
                            new Vector3(0f, -0.0046f, -0.0343f);
                        controllerAnchorOffset.localRotation =
        ...
        public virtual void SetIKTarget(Transform newAnchor)
        {
        ...
        public virtual VRInteractor GetOtherController()
        {
            VRInteractor[] interactors =
                GetVRRigRoot.GetComponentsInChildren<VRInteractor>();
        ...
        protected virtual void Start()
        {
            ...
            if (objectReference != null)
                StartCoroutine(LockObjectToController());
        }
        ...
        protected virtual IEnumerator LockObjectToController()
        {
            yield return new WaitForSeconds(0.5f);
            VRInteractableItem interactableItem = null;
            if (objectReferenceIsPrefab)
            {
                GameObject newObjectReference =
                    (GameObject) Instantiate(objectReference);
                interactableItem =
                    newObjectReference
                        .GetComponentInChildren<VRInteractableItem>();
            }
            else
                interactableItem =
                    objectReference
            ...
            if (interactableItem != null)
            {
                ...
                Debug
                    .LogWarning("Couldn't find VRInteractableItem on object reference");
        }

        protected virtual void CheckHover()
        {
            if (
                _heldItem != null //If were holding something
            )
            {
                if (
                    _highlighting //If were holding something and still highligting then stop
                )
                {
                    highlighting = false;
                }
            ...
            foreach (VRInteractableItem item in VRInteractableItem.items)
            {
                ...
                float dist =
                    Vector3.Distance(controllerPosition, targetPosition);

                if (dist > closestDist) continue;
                bool canGrab = false;
                bool isForceGrab = false;
                if (
                    dist < item.getInteractionDistance ||
                    ItemWithinColliderBounds(item)
                ) canGrab = true;
                if (
                    (
                    item.getInteractionDistance < forceGrabDistance &&
                    VRUtils
                        .PositionWithinCone(controllerPosition,
                        getControllerAnchorOffset
                            .TransformVector(new Vector3(vrInput.LeftHand
                                    ? forceGrabDirection.x
                                    : -forceGrabDirection.x,
                                forceGrabDirection.y,
                                forceGrabDirection.z)),
                        targetPosition,
                        20f,
                        forceGrabDistance)
                    )
            ...
            if (closestItem != null && (hoverItem != closestItem))
            {
                ...
                if (forceToggle != null)
                    forceToggleAllowed =
                        !forceGrab ||
                        !vrInput.ActionPressed(forceToggle.actionName);

                if (
                    _lastDropped + 0.5f < Time.time &&
                    forceToggleAllowed &&
                    (
                    vrInput.ActionPressed("ACTION") ||
                    vrInput.ActionPressed("PICKUP_DROP") ||
                    vrInput.ActionPressed("PICKUP")
                    )
                )
                {
                    hoverItem = closestItem;
                    string actionDown = "PICKUP";
                    if (vrInput.ActionPressed("PICKUP_DROP"))
                        actionDown = "PICKUP_DROP";
                    else if (vrInput.ActionPressed("ACTION"))
                        actionDown = "ACTION";
                    SendMessage("InputReceived",
                    actionDown,
        ...
        protected virtual void PositionHoverLine()
        {
            if (!useHoverLine) return;
            if (!(heldItem == null || _hoverLine == null || !_hoverLine.enabled)
            ...
            else if (_hoverLine != null && _hoverLine.enabled)
                _hoverLine.enabled = false;
        }

        protected virtual bool ItemWithinColliderBounds(VRInteractableItem item)
        {
            ...
            foreach (Collider col in item.triggerColliders)
            {
                ...
                {
                    Debug
                        .LogError("Item has an empty collider in trigger colliders list: " +
                        item.name,
                        item.gameObject);
                    continue;
                }
        ...
        {
            if (
                _heldItem != null || /*already holding something*/
                hoverItem == null || /*have something were hovering over*/
                (
                hoverItem.getHoldType !=
                VRInteractableItem.HoldType.SPRING_JOINT &&
                hoverItem.heldBy != null
                ) /*Thing were hovering over is not a joint hold and is already being held*/
            ) return false;
            _heldItem = hoverItem;
            _heldItem.DisableHover(this);
            if (
                !_heldItem.Pickup(this) /*Is the item alright with us picking it up*/
            )
            {
                _heldItem = null;
                return false;
        ...
        private void ToggleControllers(bool enable)
        {
            ...
            else
            {
                ...
                if (vrInput.LeftHand)
                {
                    ToggleAllChildRenderers(avatar.ControllerLeft.gameObject,
                    enable);
                    ToggleAllChildRenderers(avatar.HandLeft.gameObject, enable);
                }
                else
                {
                    ToggleAllChildRenderers(avatar.ControllerRight.gameObject,
                    enable);
                    ToggleAllChildRenderers(avatar.HandRight.gameObject,
                    enable);
                }
#endif
            }
